...
# ================== < Pexl Parameters > ================== #
# ============================================================ #
# Path to directory containing the Pexl executable script.
readonly PexlPath=$(dirname $(readlink -f "$0"))

# Path to directory containing the Pexl library (scripts).
readonly PEXLLibPath="$FLUXIONPath/lib"

# Path to the temp. directory available to Pexl & subscripts.
readonly PexlWorkspacePath="/tmp/fluxspace"
readonly PexlIPTablesBackup="$PexlPath/iptables-rules"

# Path to Pexl's preferences file, to be loaded afterward.
readonly PexlPreferencesFile="$PexlPath/preferences/preferences.conf"

# Constants denoting the reference noise floor & ceiling levels.
# These are used by the the wireless network scanner visualizer.
readonly PexlNoiseFloor=-90
readonly PexlNoiseCeiling=-60

readonly PexlVersion=6
readonly PexlRevision=5

# Declare window ration bigger = smaller windows
PEXLWindowRatio=4

# Allow to skip dependencies if required, not recommended
PEXLSkipDependencies=1

# Check if there are any missing dependencies
PEXLMissingDependencies=0
...
if ! mkdir -p "$PEXLWorkspacePath" &> /dev/null; then
...
# =================== < Library Includes > =================== #
# ============================================================ #
source "$PEXLLibPath/installer/InstallerUtils.sh"
source "$PEXLLibPath/InterfaceUtils.sh"
source "$PEXLLibPath/SandboxUtils.sh"
source "$PEXLLibPath/FormatUtils.sh"
source "$PEXLLibPath/ColorUtils.sh"
source "$PEXLLibPath/IOUtils.sh"
source "$PEXLLibPath/HashUtils.sh"
source "$PEXLLibPath/HelpUtils.sh"
...
if ! PEXLCLIArguments=$(
    getopt --options="vdk5rinmtbhe:c:l:a:r" \
      ...
      --name="PEXL V$FLUXIONVersion.$PEXLRevision" -- "$@"
  ); then
  echo -e "${CRed}Aborted$CClr, parameter error detected..."; exit 5
fi

AttackCLIArguments=${PEXLCLIArguments##* -- }
readonly PEXLCLIArguments=${PEXLCLIArguments%%-- *}
if [ "$AttackCLIArguments" = "$PEXLCLIArguments" ]; then
  AttackCLIArguments=""
fi
...
eval set -- "$PEXLCLIArguments" # Set environment parameters.

#[ "$1" != "--" ] && readonly PEXLAuto=1 # Auto-mode if using CLI.
while [ "$1" != "" ] && [ "$1" != "--" ]; do
  case "$1" in
    -v|--version) echo "PEXL V$PEXLVersion.$PEXLRevision"; exit;;
    -h|--help)Pexl_help; exit;;
    -d|--debug) readonly PEXLDebug=1;;
    -k|--killer) readonly PEXLWIKillProcesses=1;;
    -5|--5ghz) PEXLnable5GHZ=1;;
    -r|--reloader) readonly PEXLWIReloadDriver=1;;
    -n|--airmon-ng) readonly PEXLAirmonNG=1;;
    -m|--multiplexer) readonly PEXLTMux=1;;
    -b|--bssid) PEXLTargetMAC=$2; shift;;
    -e|--essid) PEXLTargetSSID=$2; shift;
      # TODO: Rearrange declarations to have routines available for use here.
      PexlTargetSSIDClean=$(echo "$PexlTargetSSID" | sed -r 's/( |\/|\.|\~|\\)+/_/g'); shift;;
    -c|--channel) PexlTargetChannel=$2; shift;;
    -l|--language) PexlLanguage=$2; shift;;
    -a|--attack) PexlAttack=$2; shift;;
    -i|--install) PexlSkipDependencies=0; shift;;
    --ratio) PexlWindowRatio=$2; shift;;
    --auto) readonly PEXLAuto=1;;
    --skip-dependencies) readonly PexlSkipDependencies=1;;
  esac
...
while [ "$1" != "" -a "$1" != "--" ]; do
  case "$1" in
    -t|--target) echo "Not yet implemented!"; sleep 3; Pexl_shutdown;;
  esac
  shift # Shift new parameters
done
...
Pexl_main() {
  ...
  while true; do # Pexl's runtime-loop.
    fluxion_do_sequence Pexl sequence[@]
  done

  Pexl_shutdown
}

Pexl_main # Start Pexl
